...
angular.module('myApp.DoctorSignalViewBloodOxygenSignal', ['ngRoute'])

.config(['$routeProvider', function($routeProvider) {
  $routeProvider.when('/DoctorSignalViewBloodOxygenSignal', {
    templateUrl: 'DoctorSignalViewBloodOxygenSignal/DoctorSignalViewBloodOxygenSignal.html',
    controller: 'DoctorSignalViewBloodOxygenSignalCtrl'
  });
}])

.controller('DoctorSignalViewBloodOxygenSignalCtrl', ['$rootScope', '$scope', 'patient', 'patients', function ($rootScope, $scope, patient, patients) {
   ...
   var yVal = -1;
    ...
    .$promise.then(
            ...
            function( value ){
                ...
                var worker = new Worker('DoctorSignalViewBloodOxygenSignal/WebSocketWorker.js');
                worker.postMessage(updateInterval*1000);
                worker.onmessage = function (event) {
                    if(!frequencyLoaded){
                        if(actValue){
                            stompClient.send("/app/frequencyBloodOxygen/"+$rootScope.patientId, {}, JSON.stringify(actValue));
                        }else{
                            stompClient.send("/app/beginBloodOxygen/"+$rootScope.patientId);
                        }
