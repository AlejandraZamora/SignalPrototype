...
angular.module('myApp.SignalViewECGSignal', ['ngRoute'])

.config(['$routeProvider', function($routeProvider) {
  $routeProvider.when('/SignalViewECGSignal', {
    templateUrl: 'SignalViewECGSignal/SignalViewECGSignal.html',
    controller: 'SignalViewECGSignalCtrl'
  });
}])

.controller('SignalViewECGSignalCtrl', ['$rootScope', '$scope', 'patient', '$location', function ($rootScope, $scope, patient, $location) {
    ...
    .$promise.then(
            ...
            function( value ){
                ...
                var worker = new Worker('SignalViewECGSignal/WebSocketWorker.js');
                worker.postMessage(updateInterval*1000);
                worker.onmessage = function (event) {
                    if(!frequencyLoaded){
                        if(actValue){
                            stompClient.send("/app/frequencyECG/"+$scope.patient.id, {}, JSON.stringify(actValue));
                        }else{
                            stompClient.send("/app/beginECG/"+$scope.patient.id);
                        }
