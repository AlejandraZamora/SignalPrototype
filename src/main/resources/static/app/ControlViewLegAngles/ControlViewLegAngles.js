...
angular.module('myApp.ControlViewLegAngles', ['ngRoute'])

.config(['$routeProvider', function($routeProvider) {
  $routeProvider.when('/ControlViewLegAngles', {
    templateUrl: 'ControlViewLegAngles/ControlViewLegAngles.html',
    controller: 'ControlViewLegAnglesCtrl'
  });
}])

.controller('ControlViewLegAnglesCtrl', ['$rootScope', '$scope', 'patient', function ($rootScope, $scope, patient) {
    patient.get({patientId:""+$rootScope.idPatient})
    .$promise.then(
            //success
            function( value ){
                $scope.patient=value;
                $scope.diagnostics=$scope.patient.legAngles;
$scope.upperAngle=[];
$scope.lowerAngle=[];
