...
angular.module('myApp.DoctorSignalViewECGSignal', ['ngRoute'])

.config(['$routeProvider', function($routeProvider) {
  $routeProvider.when('/DoctorSignalViewECGSignal', {
    templateUrl: 'DoctorSignalViewECGSignal/DoctorSignalViewECGSignal.html',
    controller: 'DoctorSignalViewECGSignalCtrl'
  });
}])

.controller('DoctorSignalViewECGSignalCtrl', ['$rootScope', '$scope', 'patient', 'patients', function ($rootScope, $scope, patient, patients) {
   ...
   var yVal = -1;
    ...
    function connect() {
        ...
        stompClient.connect({}, function (frame) {
            ...
            stompClient.subscribe('/topic/ECGsignaltoclient', function (greeting) {
    ...
    .$promise.then(
            ...
            function( value ){
                ...
                var worker = new Worker('DoctorSignalViewECGSignal/WebSocketWorker.js');
                worker.postMessage(updateInterval*1000);
                worker.onmessage = function (event) {
                    if(!frequencyLoaded){
                        if(actValue){
                            stompClient.send("/app/frequencyECG/"+$rootScope.patientId, {}, JSON.stringify(actValue));
                        }else{
                            stompClient.send("/app/beginECG/"+$rootScope.patientId);
                        }
